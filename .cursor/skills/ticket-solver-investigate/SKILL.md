...
2. Present **structured output**: summary, comments, Coralogix URLs, **`suggestedBranch`** / workflow hints from JSON. Keep **`suggestedBranch`** in thread context — phase 3 uses it when creating a ticket branch on a detached worktree (`git checkout -b` at current HEAD).
