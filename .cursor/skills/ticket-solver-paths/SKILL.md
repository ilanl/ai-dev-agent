...
2. **Merge base from git (for later branch commands)**  
   For each non-`N/A` root, record **`git -C <root> rev-parse --abbrev-ref HEAD`** (or equivalent) as the **current branch** to use as **`--client-base` / `--server-base`** when phase 3 creates ticket branches — **unless** the user names a different allowed base.

   **Worktrees:** agent runtimes may implement under **`.agent-runs/<thread>/worktrees/`** with **`git worktree add --detach <path> <base-sha>`**. The worktree path is detached at the base commit; still record the **base branch name** (`development`, `main`, etc.) for MR targets. Phase 3 branches with **`git -C <worktree> checkout -b <suggestedBranch>`** — see **ticket-solver-ship** — not by checking out the base branch name inside the worktree.
...
- **Agent does not** `git commit` / `git push` — unchanged from team rules.
